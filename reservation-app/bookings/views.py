...
class HomeView(LoginRequiredMixin, SingleTableView):
    queryset = Reservation.objects.annotate(previous_reservation_id=Window(expression=Lag('id', 1), partition_by=[F('rental_id')], order_by=F('check_in').asc() )).values('rental__name', 'id', 'check_in', 'check_out', 'previous_reservation_id')
